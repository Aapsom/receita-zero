...
import { generateKey as generateAEADKey, encrypt as aeadEncrypt, decrypt as aeadDecrypt, type AEADKey } from '../aead';

export interface EnvelopeConfig {
  kekEnvVar?: string;       // ENV var for KEK (default: 'DATA_KEK')
  kekKeyId?: string;        // KMS key ID (optional, for cloud KMS)
  algorithm?: 'aes-256-gcm'; // KEK algorithm (currently only AES-256-GCM)
}

export interface WrappedDEK {
  wrapped: Buffer;          // Encrypted DEK
  nonce: Buffer;            // IV for KEK encryption
  tag: Buffer;              // Auth tag for KEK encryption
  kekVersion?: string;      // KEK version for rotation tracking
}

export interface EnvelopeResult {
  ciphertext: Buffer;       // DEK-encrypted data
  nonce: Buffer;            // DEK nonce
  tag: Buffer;              // DEK auth tag
  wrappedDek: Buffer;       // DEK encrypted by KEK
  wrappedDekNonce: Buffer;  // IV for KEK encryption
  wrappedDekTag: Buffer;    // Tag for KEK encryption
  kekVersion?: string;      // KEK version for rotation tracking
}
 ...
 */
function getKEK(envVar: string): Buffer {
 ...
 */
export function generateDEK(): AEADKey {
 ...
 */
export function wrapDEK(dek: AEADKey, config: EnvelopeConfig = {}): WrappedDEK {
 ...
 */
export function unwrapDEK(wrappedDek: WrappedDEK, config: EnvelopeConfig = {}): AEADKey {
  const kek = getKEK(config.kekEnvVar || 'DATA_KEK');
 ...
 */
export function generateTenantKeys(
  tenantId: string,
  config: EnvelopeConfig = { kekEnvVar: 'DATA_KEK' }
...
export function encryptForTenant(
  ...
  config: EnvelopeConfig = { kekEnvVar: 'DATA_KEK' },
  aad?: Buffer
): Omit<EnvelopeResult, 'wrappedDek' | 'wrappedDekNonce' | 'wrappedDekTag' | 'kekVersion'> {
  // Unwrap tenant's DEK
...
export function decryptForTenant(
  envelope: Omit<EnvelopeResult, 'wrappedDek' | 'wrappedDekNonce' | 'wrappedDekTag' | 'kekVersion'> & { tenantKeys: TenantEncryptionKeys },
  config: EnvelopeConfig = { kekEnvVar: 'DATA_KEK' },
  aad?: Buffer
): Buffer {
...
export function rotateKEK(
  ...
  oldConfig: EnvelopeConfig,
  newConfig: EnvelopeConfig
): WrappedDEK[] {
