...
export interface BlindIndexResult {
  index: Buffer;            // Truncated HMAC output
  indexHex: string;         // Hex string for DB storage/query
  fullHmac: Buffer;         // Full HMAC (for verification)
}

/**
 * Get blind index key from environment
 */
function getBlindIndexKey(envVar: string): Buffer {
 ...
 */
export function generateBlindIndex(
 ...
 */
export function generateCompositeBlindIndex(
 ...
 */
export function blindIndexForSearch(
  searchTerm: string | Buffer,
  config: BlindIndexConfig = {}
): BlindIndexResult {
 ...
 */
export function generateBlindIndexKey(): string {
 ...
 */
export function rotateBlindIndexKey(
  plaintexts: (string | Buffer)[],
  oldConfig: BlindIndexConfig,
  newConfig: BlindIndexConfig
): Map<string, string> {
