 ...
 */
export async function verifyPassword(password, hash, config) {
    if (!argon2) {
    ...
    try {
        ...
        if (config?.associatedData)
            options.associatedData = config.associatedData;
 ...
 */
export function needsRehash(hash, config = {}) {
    if (!argon2) {
 ...
 */
export function getPepper(envVar = 'PASSWORD_PEPPER') {
 ...
 */
export async function calibrateArgon2(targetTimeMs = 100, maxMemoryKiB = 65536) {
    if (!argon2) {
        throw new Error('argon2 module not available');
    }
